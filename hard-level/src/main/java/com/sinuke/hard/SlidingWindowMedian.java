...
public class SlidingWindowMedian {
    ...
    public double[] medianSlidingWindow(int[] nums, int k) {
        ...
        for (int i = k; i <= nums.length; i++) {
            double sum = 0.0;
            if (even) sum = (findSmallest(root, k/2).val + findSmallest(root, k/2 + 1).val) / 2.0;
            else sum = findSmallest(root, k/2 + 1).val;
    ...
    private Node findSmallest(Node root, int k) {
        ...
        if (s == k) return root;
        if (s > k) return findSmallest(root.left, k);
    ...
    private Node updateNode(Node root) {
        ...
        } else if (b == 2) root = rightRotate(root);
        else if (b == -2) root = leftRotate(root);
