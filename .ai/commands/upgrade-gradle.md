...
Upgrade Gradle dependencies, plugins, jacoco toolVersion, and the Gradle wrapper to their latest **stable** versions (no alpha, beta, RC, or milestone releases). After updating, if the MySQL version changed, sync the Docker image tag in `SQLSolutionsTest.java`.
...
List all files modified. Then ask the user to confirm:

- "Run tests?" — Yes: run `./gradlew clean test` and report results. No: stop here, leave testing to the user.
...
If tests **passed** and there were updates applied, ask the user to confirm:

- "Commit changes?" — Yes: commit all updated files with a dependency upgrade message. No: stop here, leave committing to the user.

If the user declines, stop here.

If the user agrees, stage all modified files and commit:
